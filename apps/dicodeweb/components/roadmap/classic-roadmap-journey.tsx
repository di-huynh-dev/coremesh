...
function buildTopicDistribution(totalTopics: number, milestoneCount: number) {
  ...
  return Array.from({ length: milestoneCount }, (_, index) => baseTopics + (index < remainder ? 1 : 0));
}
...
function VerticalJourneyPath({ milestoneCount, accent }: { milestoneCount: number; accent: string }) {
  ...
  return (
    ...
    >
      ...
      <path d={d} fill="none" stroke={accent} strokeOpacity="0.55" strokeWidth="10" strokeLinecap="round" />
    </svg>
  );
}
...
function OverviewJourneyTrack({
  ...
  return (
    <svg className="h-24 w-full" viewBox={`0 0 ${width} ${height}`}>
      ...
      <path d={path} fill="none" stroke={accent} strokeOpacity="0.65" strokeWidth="3" strokeLinecap="round" />
      {points.map((point, index) => {
        ...
        return (
          <g key={`${point.x}-${point.y}`}>
            <circle cx={point.x} cy={point.y} r={isActive ? 18 : 13} fill={isActive ? '#FFFFFF' : isCompleted ? `${accent}1A` : '#FFFFFF'} stroke={isActive || isCompleted ? accent : '#D9D2C7'} strokeWidth={isActive ? 4 : 3} />
            <text
              x={point.x}
              y={point.y + 4}
              textAnchor="middle"
...
function MilestonePreview({
  ...
  if (variant === 1) {
    return (
      <div className="rounded-[1.2rem] p-3 font-mono text-[11px] leading-5 text-white" style={{ background: strong }}>
  ...
  return (
    <div className="rounded-[1.2rem] border border-border/70 bg-white/80 p-3 dark:bg-white/5">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="rounded-full px-2 py-1 text-[10px] font-bold tracking-[0.16em] uppercase" style={{ background: `${accent}18`, color: accent }}>
          #{milestone.step}
        </div>
        <div className="h-px flex-1" style={{ background: `${accent}45` }} />
      </div>
  ...
  return (
    <article className="paper-card rounded-[1.65rem] p-5 shadow-[0_18px_60px_rgba(15,68,122,0.08)]">
      <div className="grid gap-4 xl:grid-cols-[1fr_168px] xl:items-start">
        <div>
          <h4 className="text-lg font-semibold text-[#071B3A] dark:text-[#F5F0EA]">{milestone.title}</h4>
          ...
          <div className="mt-5 flex items-center justify-between gap-3">
            <span className="text-sm font-medium text-muted-foreground">{topics} {topicUnitLabel}</span>
            <span
        ...
        <MilestonePreview milestone={milestone} accent={roadmap.theme.accent} strong={roadmap.theme.strong} index={index} />
      </div>
    </article>
  );
}
...
export function ClassicRoadmapJourney({ roadmapId, roadmap, content, copy }: ClassicRoadmapJourneyProps) {
  const topicsByMilestone = buildTopicDistribution(roadmap.totals.topics, content.milestones.length);
  const phaseRanges = buildPhaseRanges(content.milestones.length, copy.detail.phaseLabels, copy.detail.totalLabel);
  const activeIndex = Math.min(content.milestones.length - 1, Math.floor(content.milestones.length / 2));
    ...
    <div className="space-y-8">
      <div className="grid gap-6 lg:grid-cols-[1.02fr_0.98fr] lg:items-start">
        <div className="space-y-5 rounded-[2rem] bg-white/55 p-1 dark:bg-white/0">
          <div className="inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold" style={{ borderColor: roadmap.theme.border, background: roadmap.theme.soft, color: roadmap.theme.accent }}>
          ...
          <div className="space-y-4">
            ...
            <p className="max-w-2xl text-base leading-7 text-muted-foreground md:text-lg">{content.summary}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 pt-2">
            {[content.durationText, content.topicCountText, content.questionCountText].map((stat) => (
        ...
        <div className="paper-card rounded-[1.85rem] p-5 shadow-[0_18px_60px_rgba(15,68,122,0.08)] md:p-6">
          <h3 className="text-lg font-semibold text-[#071B3A] dark:text-[#F5F0EA]">{copy.detail.overviewTitle}</h3>

          <div className="mt-4">
            <OverviewJourneyTrack milestoneCount={content.milestones.length} activeIndex={activeIndex} accent={roadmap.theme.accent} />
          </div>

          <div className="mt-4 grid gap-3 sm:grid-cols-4">
            {phaseRanges.map((phase) => (
              <div key={`${phase.label}-${phase.range}`} className="rounded-[1.15rem] border border-border/70 bg-white/75 px-4 py-3 dark:bg-white/5">
                <p className="text-sm font-semibold text-[#071B3A] dark:text-[#F5F0EA]">{phase.label}</p>
          ...
          <div className="mt-5 flex flex-col gap-3 rounded-[1.25rem] border px-4 py-3 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: roadmap.theme.border, background: roadmap.theme.soft }}>
            <p className="text-sm font-semibold" style={{ color: roadmap.theme.accent }}>
              {copy.detail.currentMilestoneLabel} {activeMilestone.step}: {activeMilestone.title}
            </p>
            <a href="#classic-roadmap-path" className="inline-flex items-center gap-2 text-sm font-semibold" style={{ color: roadmap.theme.accent }}>
              {copy.detail.continueAction}
              <ArrowRight className="h-4 w-4" />
            </a>
          </div>
      ...
      <div id="classic-roadmap-path" className="pt-2">
        <h3 className="mb-6 text-2xl font-bold tracking-tight text-[#071B3A] dark:text-[#F5F0EA]">{copy.detail.milestonesTitle}</h3>

        <div className="space-y-4 lg:hidden">
          {content.milestones.map((milestone, index) => (
            <div key={`${roadmapId}-mobile-${milestone.step}`} className="relative pl-16">
              {index < content.milestones.length - 1 ? (
                <div className="absolute top-11 left-[1.45rem] h-[calc(100%-0.5rem)] w-[6px] rounded-full" style={{ background: `${roadmap.theme.accent}2B` }} />
              ) : null}
        ...
        <div className="relative hidden lg:block" style={{ minHeight: content.milestones.length * DESKTOP_NODE_HEIGHT }}>
          <VerticalJourneyPath milestoneCount={content.milestones.length} accent={roadmap.theme.accent} />
          ...
          <div className="relative z-10">
            {content.milestones.map((milestone, index) => {
              ...
              return (
                <div key={`${roadmapId}-${milestone.step}`} className={`flex items-center ${isLeft ? 'flex-row' : 'flex-row-reverse'}`} style={{ minHeight: DESKTOP_NODE_HEIGHT }}>
                  <div className="w-[43%]">
                    <DesktopMilestoneCard
                      milestone={milestone}
                      index={index}
                  ...
                  <div className="relative flex w-[14%] shrink-0 items-center justify-center">
                    <div className="absolute top-1/2 left-0 right-0 h-px -translate-y-1/2 bg-border/80" />
                    <div
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="paper-card rounded-[1.6rem] p-5">
          <h3 className="text-xl font-bold text-[#071B3A] dark:text-[#F5F0EA]">{content.support.title}</h3>
          <p className="mt-3 text-sm leading-7 text-muted-foreground">{content.support.description}</p>

          <div className="mt-4 space-y-3">
            {content.support.items.map((item) => (
              <div key={item} className="flex items-start gap-3 rounded-[1rem] border border-border/70 bg-white/75 px-4 py-3 dark:bg-white/5">
                <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0" style={{ color: roadmap.theme.accent }} />
        ...
        <div className="paper-card rounded-[1.6rem] p-5">
          <h3 className="text-xl font-bold text-[#071B3A] dark:text-[#F5F0EA]">{copy.detail.outcomesTitle}</h3>
          <div className="mt-4 space-y-3">
            {content.outcomes.map((outcome) => (
              <div key={outcome} className="flex items-start gap-3">
                <div className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: roadmap.theme.accent }} />
        ...
        <div className="paper-card rounded-[1.6rem] p-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold text-[#071B3A] dark:text-[#F5F0EA]">{copy.detail.skillMapTitle}</h3>
          </div>
